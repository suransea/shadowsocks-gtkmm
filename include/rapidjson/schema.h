      ...
      if (properties && properties->IsObject()) {
          ...
          for (ConstMemberIterator itr = properties->MemberBegin(); itr != properties->MemberEnd(); ++itr) {
              ...
              if (FindPropertyIndex(itr->name, &index))
                  schemaDocument
                      ->CreateSchema(&properties_[index].schema, q.Append(itr->name, allocator_), itr->value, document);
          }
      }
      ...
      if (const ValueType *v = GetMember(value, GetItemsString())) {
          ...
          if (v->IsObject()) // List validation
              ...
              for (ConstValueIterator itr = v->Begin(); itr != v->End(); ++itr, index++)
                  schemaDocument
                      ->CreateSchema(&itemsTuple_[itemsTupleCount_++], q.Append(index, allocator_), *itr, document);
          }
      }
 ...
 private:
  ...
  bool CreateParallelValidator(Context &context) const {
      ...
      if (validatorCount_) {
          ...
          context.validators = static_cast<ISchemaValidator **>(context.factory
              .MallocState(sizeof(ISchemaValidator *) * validatorCount_));
  ...
  void CreateSchema(const SchemaType **schema,
      ...
      if (v.IsObject()) {
          if (!HandleRefSchema(pointer, schema, v, document)) {
              SchemaType
                  *s = new(allocator_->Malloc(sizeof(SchemaType))) SchemaType(this, pointer, v, document, allocator_);
          ...
          if (len > 0) {
              ...
              if (i > 0) { // Remote reference, resolve immediately
                  ...
                  if (pointer.IsValid()) {
                      ...
                      new(schemaRef_.template Push<SchemaRefEntry>()) SchemaRefEntry(source,
                                                                                     pointer,
                                                                                     schema,
                                                                                     allocator_);
                      return true;
...
class GenericSchemaValidator :
  ...
  */
  ...
  PointerType GetInvalidDocumentPointer() const {
      return documentStack_.Empty() ? PointerType() : PointerType(documentStack_.template Bottom<Ch>(),
...
#define RAPIDJSON_SCHEMA_HANDLE_VALUE_(method, arg1, arg2) \
  ...
  bool RawNumber(const Ch *str, SizeType length, bool copy) {
      RAPIDJSON_SCHEMA_HANDLE_VALUE_(String,
                                     (CurrentContext(), str, length, copy),
                                     (str, length, copy));
  }
...
#undef RAPIDJSON_SCHEMA_HANDLE_VALUE_
  ...
  virtual ISchemaValidator *CreateSchemaValidator(const SchemaType &root) {
      return new(GetStateAllocator().Malloc(sizeof(GenericSchemaValidator))) GenericSchemaValidator(*schemaDocument_,
                                                                                                    root,
#if RAPIDJSON_SCHEMA_VERBOSE
          depth_ + 1,
#endif
 ...
 private:
  ...
  bool EndValue() {
      ...
      *documentStack_.template Push<Ch>() = '\0';
      ...
      uint64_t
          h = CurrentContext().arrayUniqueness ? static_cast<HasherType *>(CurrentContext().hasher)->GetHashCode() : 0;
  ...
  RAPIDJSON_FORCEINLINE void PushSchema(const SchemaType &schema) {
      new(schemaStack_.template Push<Context>()) Context(*this,
                                                         &schema);
  }
