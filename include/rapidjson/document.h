...
class GenericValue {
  ...
  ~GenericValue() {
      if (Allocator::kNeedFree) { // Shortcut by Allocator's trait
          switch (data_.f.flags) {
              ...
              case kCopyStringFlag:Allocator::Free(const_cast<Ch *>(GetStringPointer()));
                  break;
  ...
  */
  ...
  RAPIDJSON_DISABLEIF_RETURN((internal::OrExpr<internal::IsPointer<T>, internal::IsGenericValue<T> >),
                             (bool)) operator==(const T &rhs) const { return *this == GenericValue(rhs); }
   ...
   */
  template<typename T>
  RAPIDJSON_DISABLEIF_RETURN((internal::IsGenericValue<T>), (bool)) operator!=(const T &rhs) const {
      return !(*this == rhs);
  }
  ...
  */
  ...
  template<typename T>
  RAPIDJSON_DISABLEIF_RETURN((internal::NotExpr<internal::IsSame<typename internal::RemoveConst<T>::Type, Ch> >),
                             (const GenericValue&)) operator[](T *name) const { return const_cast<GenericValue &>(*this)[name]; }
  ...
  */
  ...
  template<typename SourceAllocator>
  const GenericValue &operator[](const GenericValue<Encoding,
                                                    SourceAllocator> &name) const { return const_cast<GenericValue &>(*this)[name]; }
  ...
  */
  ...
  template<typename SourceAllocator>
  ConstMemberIterator FindMember(const GenericValue<Encoding,
                                                    SourceAllocator> &name) const {
  ...
  GenericValue &AddMember(GenericValue &name, GenericValue &value, Allocator &allocator) {
      ...
      if (o.size >= o.capacity) {
          if (o.capacity == 0) {
              ...
              SetMembersPointer(reinterpret_cast<Member *>(allocator
                  .Realloc(GetMembersPointer(), oldCapacity * sizeof(Member), o.capacity * sizeof(Member))));
          }
      }
  ...
  */
  GenericValue &Reserve(SizeType newCapacity, Allocator &allocator) {
      ...
      if (newCapacity > data_.a.capacity) {
          SetElementsPointer(reinterpret_cast<GenericValue *>(allocator.Realloc(GetElementsPointer(),
  ...
  */
  ...
  ValueType &Set(const T &data, AllocatorType &allocator) {
      return internal::TypeHelper<ValueType, T>::Set(*this,
                                                     data,
                                                     allocator);
  }
      ...
      switch (GetType()) {
          ...
          case kObjectType:
              ...
              for (ConstMemberIterator m = MemberBegin(); m != MemberEnd(); ++m) {
                  ...
                  if (RAPIDJSON_UNLIKELY(!handler
                      .Key(m->name.GetString(), m->name.GetStringLength(), (m->name.data_.f.flags & kCopyFlag) != 0)))
                      return false;
          ...
          case kStringType:return handler.String(GetString(), GetStringLength(), (data_.f.flags & kCopyFlag) != 0);

          default:RAPIDJSON_ASSERT(GetType() == kNumberType);
 ...
 private:
  ...
  RAPIDJSON_FORCEINLINE GenericValue *GetElementsPointer() const {
      return RAPIDJSON_GETPOINTER(GenericValue,
                                  data_.a.elements);
  }
...
GenericValue<Encoding, Allocator>::GenericValue(const GenericValue<Encoding, SourceAllocator> &rhs,
    ...
    switch (rhs.GetType()) {
        ...
        case kStringType:
            ...
            break;
        default:data_.f.flags = rhs.data_.f.flags;
            data_ = *reinterpret_cast<const Data *>(&rhs.data_);
            break;
    }
}
...
class GenericArray {
 public:
  ...
  template<typename T>
  RAPIDJSON_DISABLEIF_RETURN((internal::OrExpr<internal::IsPointer<T>, internal::IsGenericValue<T> >),
                             (const GenericArray&)) PushBack(T value, AllocatorType &allocator) const {
      value_.PushBack(value, allocator);
      return *this;
  }
...
class GenericObject {
 public:
  ...
  template<typename T>
  RAPIDJSON_DISABLEIF_RETURN((internal::OrExpr<internal::IsPointer<T>, internal::IsGenericValue<T> >),
                             (ValueType & )) AddMember(ValueType &name,
                                                       T value,
